...
В обычном `.xlsx` шаблоне даты форматируются без макросов. Для этого книга использует скрытый технический лист `Trino Schema`: он получает из Trino типы колонок через отдельный schema-запрос без строк результата и помогает Excel показывать `date`, `timestamp` и `timestamp with time zone` как даты/дату-время, а не как общий числовой формат.
...
4. создает скрытую schema-таблицу для форматирования дат в `.xlsx`;
5. создает таблицу результата на листе `Trino Result`;
6. сохраняет готовый шаблон.
